...
type, extends(cPairModel) :: pair_lj_cut_coul_cut
  ...
  contains
    ...
    procedure :: virial => pair_lj_cut_coul_cut_virial
...
!---------------------------------------------------------------------------------------------------
  ...
  function pair_lj_cut_coul_cut_virial( model, invR2, Qi, Qj ) result( Wij )
    class(pair_lj_cut_coul_cut), intent(in) :: model
    real(rb),                    intent(in) :: invR2, Qi, Qj
    real(rb)                                :: Wij
    ...
    sr6 = sr2*sr2*sr2
    sr12 = sr6*sr6
    Wij = model%eps24*(sr12 + sr12 - sr6) + Qi*Qj*sqrt(invR2)

  end function pair_lj_cut_coul_cut_virial
