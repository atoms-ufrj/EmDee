...
type, extends(cPairModel) :: pair_softcore_sf_coul_sf
  ...
  contains
    ...
    procedure :: virial => pair_softcore_sf_coul_sf_virial
...
!---------------------------------------------------------------------------------------------------
  ...
  function pair_softcore_sf_coul_sf_virial( model, invR2, Qi, Qj ) result( Wij )
    class(pair_softcore_sf_coul_sf), intent(in) :: model
    real(rb),                        intent(in) :: invR2, Qi, Qj
    real(rb)                                    :: Wij

    real(rb) :: rsig2, rsig6, sinv, sinvSq, sinvCb, invR, QiQj
    ...
    Wij = model%prefactor6*rsig6*(sinvCb + sinvCb - sinvSq) + &
          QiQj*invR - (model%fshift_vdw + QiQj*model%fshift_coul)/invR

  end function pair_softcore_sf_coul_sf_virial
